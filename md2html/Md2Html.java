...
public class Md2Html {
    public static Map<String, String> converter = new HashMap<>(Map.of("*", "em",
                                                                        "**", "strong", 
                                                                        "_", "em", 
                                                                        "__", "strong", 
                                                                        "--", "s", 
    ...
    public static Map<Character, String> converterForBadSymbols = new HashMap<>(Map.of('<', "&lt;",
                                                                                        '>', "&gt;", 
                                                                                        '&', "&amp;"));    
    ...
    private static int rangOfHead (String line) {
        int rang = 0;
    ...
    private static void makeEnd (StringBuilder tmpLine, int rang) {
    ...
    private static void makeStart (StringBuilder tmpLine, int rang) {
        if (rang > 0) {
    ...
    private static void makeLineHTML (StringBuilder tmpLine, Deque<String> symbolStack) {
        int j = 0;
        while (j < tmpLine.length()) {
            ...
            if (isGoodSymbol(ch)) {
                ...
                if (j > 0 
                    && Character.isWhitespace(tmpLine.charAt(j - 1)) 
                    && (( j + string.length() < tmpLine.length() 
                        && Character.isWhitespace(tmpLine.charAt(j + string.length())) ) 
                        || j + string.length() == tmpLine.length())) {
                    j += string.length();
                    continue;
                }
    ...
    public static void makeHTML(String input, String output) {
        try {
            BufferedReader in = new BufferedReader(new InputStreamReader(new FileInputStream(new File(input)), "UTF-8"));
            try {
